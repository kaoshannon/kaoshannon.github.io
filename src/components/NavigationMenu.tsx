...
const NavigationMenu: React.FC = () => {
  ...
  return (
    // TODO: refactor into one return component with tailwind responsive classes
    <div className="fixed z-menu bottom-4 right-4 sm:bottom-32 sm:right-6 lg:right-8 lg:top-1/2 lg:transform lg:-translate-y-1/2">
      ...
      <nav className="hidden sm:block space-y-2 sm:space-y-4 lg:space-y-6">
        ...
        <button
          onClick={() => window.open("https://poppp.substack.com/", "_blank")}
          rel="noopener noreferrer"
          className="block text-right font-serif italic text-sm sm:text-base lg:text-lg text-white hover:text-yellow-200 transition-colors"
        >
          writer's room →
        </button>
        {/* TODO: add a listening room with interactive vinyls hehe */}
        {/* <button
      ...
      <div className="sm:hidden" ref={menuRef}>
        ...
        <div
          id="mobile-nav"
          className={`origin-bottom-right absolute bottom-16 right-0 mt-2 w-48 p-0 transform transition-all ${
            open
        ...
        >
          <nav className="space-y-2">
            ...
            <button
              onClick={() =>
                window.open("https://poppp.substack.com/", "_blank")
              }
              rel="noopener noreferrer"
              className="block text-right font-serif italic text-sm text-white w-full"
            >
